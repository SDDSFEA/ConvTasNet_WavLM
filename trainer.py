...
class Trainer():
    ...
    def __init__(self,
        ...
        if resume['resume_state']:
            # best_ckpt_path = os.path.join(resume['path'], self.checkpoint, 'last.pt')
            ...
            if os.path.exists(best_ckpt_path):
                ...
                # cpt = torch.load("/home/student/zt/Conv-TasNet/Conv_TasNet_Pytorch/Conv-TasNet-Noise/best_N.pt", map_location='cpu')
                cpt = torch.load("/home/student/zt/ConvTasNet_master/baseline_checkpoint/best.pt", map_location='cpu')
                ...
                state_dict = torch.load("wavlm_large_pretrained/pytorch_model.bin", map_location='cpu')
                # HuggingFace权重key不带encoder.前缀，需要加上以匹配自定义WavLMencoder
                state_dict = {"encoder." + k: v for k, v in state_dict.items()}
                # state_dict = load_file("/home/student/zt/ConvTasNet_master/baseline_checkpoint/encoder_1b_noisy.safetensors")
