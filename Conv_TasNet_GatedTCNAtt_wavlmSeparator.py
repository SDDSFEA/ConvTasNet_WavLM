...
class WavLMencoder(PreTrainedModel):
    supports_gradient_checkpointing = True
    def __init__(
        self,
        wavlm_name: str = "microsoft/wavlm-base",
        talker_numbers: int = 2,
        separator_hidden: int = 256,
        # decoder_dim: int = 256,
        # **kwargs,  # 可以接收 trainer 传来的多余参数
    ):
        # 初始化 config
...
class SeparationModule(nn.Module):
    def __init__(self, R, X, in_channels, out_channels,
             kernel_size, norm, causal, semantic_dim, attn_dim):
        super().__init__()
...
class ConvTasNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # wavlm_out: List[[B, T_w, D]]
        wavlm_out = self.wavlm_encoder(input_values=x)
        # print(f"len(wavlm_out): {len(wavlm_out)}") 
