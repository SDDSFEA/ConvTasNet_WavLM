...
class GatedCrossAttnAdapter(nn.Module):
    """
    Gated Tiny Cross-Attention Adapter for TCN features
    x:          [B, C, T]   (audio bottleneck feature)
    text_embed: [B, D]      (sentence embedding)
    """
    def __init__(
        self,
        audio_dim: int,
        ...
        # TCN-style normalization (channel-wise)
        self.norm = select_norm(norm_type, audio_dim)
        # Global gate (start closed)
        self.gate_logit = nn.Parameter(torch.tensor(-2.0))  # sigmoid ≈ 0.12
    ...
    def forward(self, x, text_embed):
        ...
        # ---- tiny cross-attention ----
        Q = self.q_proj(x_t)                  # [B, T, A]
        K = self.k_proj(text_embed)           # [B, 1, A]
        V = self.v_proj(text_embed)           # [B, 1, A]
...
class ConvTasNet(nn.Module):
    ...
    def forward(self, x, text1, text2):
        if x.dim() >= 3:
            raise RuntimeError(
        ...
        # 新增：语义注入
        # 文本编码
        text_embed1 = self.text_encoder(text1)
        text_embed2 = self.text_encoder(text2)
        # 合并文本特征
        text_embed = torch.cat([text_embed1, text_embed2], dim=1)
        # n x B x L => n x B x L
