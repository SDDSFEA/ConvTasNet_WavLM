...
class Trainer():
    ...
    def train(self, train_dataloader):
        ...
        for egs in train_dataloader:
            ...
            processed_egs = {}
            for key, value in egs.items():
                if isinstance(value, torch.Tensor):
                    # print("key",key)
                    processed_egs[key] = value.to(self.device)
                else:
                    processed_egs[key] = value
            egs = processed_egs
            self.optimizer.zero_grad()
            ests = data_parallel(self.net, (egs['mix'],egs['transcript1'],egs['transcript2']), device_ids=self.gpuid)
